const Home = {
        ...
        async getBoard() {
          try {
            ...
            if (result.data && result.data.result == 'success') {
                ...
                this.middleItems = this.boardItems.filter((elem => elem.attr1.indexOf('중간') > -1))
                this.mainTitles = this.boardItems.filter((elem => elem.attr1.indexOf('타이틀') > -1))
    ...
    template: `
        ...
        <div class="py-5 container-xxl t-pos">
            ...
            <div v-if="middleItems.length > 0" class="row g-3 row-cols-1 row-cols-sm-2 row-cols-lg-3 row-cols-xxl-4">

                <div v-for="(item, idx) in middleItems" :key="item.bno" class="col">

                    <div class="card shadow-sm">
                        
                        <div class="card-header fs-5 fw-bold">{{ item.subject }}</div>
                        ...
                        <div class="card-body">
                            ...
                            <!--<div class="d-flex justify-content-between align-items-center">
                                ...
                                <router-link :to="item.linkUrl" class="btn btn-primary">바로가기</router-link>
                                </template>
                                <template v-else>
                                <a @click.prevent="click" :href="item.linkUrl" class="btn btn-primary">바로가기</a>
                                </template>
