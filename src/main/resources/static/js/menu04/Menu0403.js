    ...
    template: `
        <main class="container">
          ...
          <div class="my-3 p-3 bg-body rounded shadow-sm">
            ...
            <template v-else="">
            <bd-item v-for="item in boardItems" :key="item.bno" :kind="'MENU0403'" :bno="item.bno" :subject="item.subject" :content="item.content" :tagYn="item.tag_yn" :thumbYn="'N'"></bd-item>
            </template>
    
            <div v-if="nextYn == 'Y' ? true : false" class="d-flex mt-2 justify-content-center">
              <router-link class="btn btn-outline-primary col-12 col-md-3" role="button" :to="{name: 'MENU0403', query: {pageno:pageNextNo}}">더보기</router-link>
            </div>
          </div>
        </main>
        `
